  ...
  gps: {
    ...
    setup: {
      ...
      steps: [
        ...
        {
          name: "デベロッパイメージを準備",
          text: "「準備」をクリックします。GeoSpoof GPS が Apple のデベロッパディスクイメージを iPhone にマウントします。これが端末の実際の位置を設定できるようにする要素です。",
          bullets: [
            "これは何か: Apple が署名した小さなシステムイメージ（DDI）で、iPhone に実際の GPS 位置を設定するなどのデベロッパ機能を有効にします。",
            "入手方法: Xcode に同梱されています。Mac App Store から Xcode を無料でインストールし、iPhone を接続した状態で一度開いてください。GeoSpoof GPS がそのコピーを見つけて利用します。プロジェクトもビルドも不要で、コンパイルするものはありません。",
            "自分のイメージを使う: すでにデベロッパイメージがある場合は、そのフォルダを GeoSpoof GPS に指定すれば、Xcode のダウンロードは不要です。",
          ],
