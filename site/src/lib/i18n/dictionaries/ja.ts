...
export const ja: Dictionary = {
  ...
  comparison: {
    ...
    subhead:
      "ほとんどの位置情報偽装ツールは1つのことしかしません。偽の緯度と経度をブラウザに入れるだけです。GeoSpoof はシグナル全体をカバーするので、位置情報・タイムゾーン・IP がすべて同じ内容を示します。しかも中心となる機能は無料です。",
    featureHeader: "機能",
    ...
    features: {
      ...
      oneIdentity: "数十のブラウザ API にわたる一貫した1つのアイデンティティ",
    ...
    troubleshooting: {
      ...
      steps: [
        ...
        },
        {
          title: "最新バージョンに更新する",
          featured: false,
          body: "多くの問題は、より新しいリリースで既に修正されています。ポップアップでヘッダーの歯車アイコンから「設定」を開いてバージョンを確認し、下の最新リリースと比較して、遅れている場合はブラウザの拡張機能マネージャーから更新してください。",
          details: [],
          note: "",
  ...
  gps: {
    ...
    experimental: {
      label: "実験的機能",
      title: "初期段階の実験的な機能です",
      body: "GeoSpoof GPS は新しく、さまざまな端末での検証がまだ進行中です。細かな不具合や、初回のみ必要な設定があることをご了承ください。これは任意の追加機能で、GeoSpoof の他の機能はこれなしでも問題なく動作します。",
    },
    hero: {
      breadcrumbHome: "ホーム",
    ...
    setup: {
      ...
      steps: [
        {
          name: "アプリをインストール",
          text: "DMG を開き、GeoSpoof GPS を「アプリケーション」フォルダにドラッグして起動します。メニューバーから動作し、Dock アイコンもウインドウもありません。初回起動時にセットアップウィザードが開きます。",
        },
        {
          name: "ローカルネットワークへのアクセスを許可",
          text: "初回起動時に、macOS がローカルネットワークへのアクセスを求めます。「許可」をクリックすると、GeoSpoof GPS が iPhone を見つけて通信できるようになります。許可しないと、アプリは iPhone を認識できません。",
          bullets: [
        ...
        {
          name: "Xcode をインストール",
          text: "GeoSpoof GPS には Apple のデベロッパディスクイメージが必要で、これは Xcode に同梱されています。まずこのダウンロードを始めてください。サイズが大きい（数 GB）ため、下の短い iPhone の手順を進める間にバックグラウンドでインストールさせておきましょう。",
          bullets: [
          ...
          link: { label: "Mac App Store で Xcode を入手" },
        },
        {
          name: "iPhone を接続",
          text: "iPhone を Mac に接続し、ロックを解除します。充電専用のものもあるため、データ通信対応のケーブルを使ってください。表示されない場合は、別のケーブルやポートを試してください。",
        },
        {
          name: "このコンピュータを信頼",
          text: "iPhone にこのコンピュータを信頼するか尋ねられたら、「信頼」をタップしてパスコードを入力します。これで Mac と iPhone が通信できるようになります。",
          bullets: [
        ...
        {
          name: "デベロッパモードを有効化",
          text: "iPhone で 設定 ▸ プライバシーとセキュリティ ▸ デベロッパモード を開いてオンにし、求められたら再起動します。これは iPhone を Mac に一度接続した後にのみ表示されます。",
        },
        {
          name: "この Mac とペアリング",
          text: "セットアップウィンドウで「ペアリング」をクリックします。この一度きりの安全なハンドシェイクにより、Mac が iPhone の GPS を制御できるようになります。実行中は iPhone のロックを解除し、接続したままにしてください。",
        },
        {
          name: "デベロッパイメージを準備",
          text: "Xcode のインストールが必要です（上記）。iPhone を接続してロックを解除した状態で「準備」をクリックします。GeoSpoof GPS が Apple のデベロッパディスクイメージ（DDI）をあなたの端末向けに個別化してマウントします。これが、アプリが実際の GPS 位置を設定できるようにする要素です。Apple 署名済みで自動的に行われ、設定するものは何もありません。",
          powerUserNote:
    ...
    howItWorks: {
      title: "仕組み",
      intro:
        "ブラウザとiPhoneで一貫した位置情報を、すでにお使いのGeoSpoofアプリからまとめて操作します。",
      steps: [
        {
          title: "あなたが選び、あとは追従",
        ...
        },
        {
          ...
          body: "アプリは位置を固定し続け、新しい場所を選ぶと即座に切り替えます。同期を一時停止するかアプリを終了すれば、iPhoneはすぐに本来のGPSに戻ります——スプーフは一切残りません。",
        },
      ],
    ...
    connection: {
      ...
      offlineCaveat:
        "ただし、その位置は固定されます。別の位置を選ぶには、デベロッパモードをもう一度オンにし、iPhone を再起動して、パソコンが近くにある状態で新しい位置を設定してください。",
      limitTitle: "できないこと",
      limitBody:
    ...
    help: {
      title: "解決しませんか？",
      body: "手順が完了しない場合は、サポートページに他の対処法があります。バグを見つけた、またはアイデアがありますか？ ぜひお聞かせください。",
      supportLink: "ヘルプを見る",
