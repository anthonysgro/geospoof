...
describe("Status Display Functions", () => {
  ...
  const displayLocationStatus = (location: Location | null | undefined) => {
    if (!location) {
  ...
  const displayTimezoneStatus = (timezone: Timezone | null | undefined) => {
    if (!timezone) {
      return "Not configured";
    }
  ...
  const displayOverriddenApis = (enabled: boolean, hasLocation: boolean, hasTimezone: boolean) => {
    if (!enabled) {
  ...
  const displayWebRTCStatus = (enabled: boolean) => {
    return enabled ? "✓ Active (non-proxied UDP disabled)" : "✗ Inactive";
  };
