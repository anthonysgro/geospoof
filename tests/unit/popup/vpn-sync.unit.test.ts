...
describe("Popup HTML - VPN Sync Elements", () => {
  beforeEach(() => {
    const html = fs.readFileSync(path.join(__dirname, "../../../assets/popup.html"), "utf8");
    document.documentElement.innerHTML = html;
  });
  ...
  /**
   * Test: VPN sync toggle exists; old VPN tab removed
   * Validates: Requirements 3.1, 5.1 (updated for vpn-toggle-ui)
   */
  test("should have VPN sync toggle and no old VPN tab", () => {
    const searchTab = document.getElementById("searchModeTab");
    const coordsTab = document.getElementById("coordsModeTab");
    const vpnToggle = document.getElementById("vpnSyncToggle");
    const vpnTab = document.getElementById("vpnSyncModeTab");

    expect(searchTab).toBeTruthy();
    expect(coordsTab).toBeTruthy();
    expect(vpnToggle).toBeTruthy();
    expect(vpnTab).toBeNull();
  });

  /**
   * Test: VPN sync panel has all required elements
   * Validates: Requirements 3.3, 3.6, 5.1
   */
  test("should have VPN sync panel with status, error, and action elements", () => {
    const panel = document.getElementById("vpnSyncMode");
    const status = document.getElementById("vpnSyncStatus");
    const ipDisplay = document.getElementById("vpnSyncIp");
    const errorDisplay = document.getElementById("vpnSyncError");
    const syncBtn = document.getElementById("vpnSyncButton");
    const resyncBtn = document.getElementById("vpnResyncButton");
  ...
  /**
   * Test: VPN sync panel is initially hidden
   * Validates: Requirements 3.1
   */
  test("should have VPN sync panel initially hidden", () => {
    const panel = document.getElementById("vpnSyncMode");
    expect(panel?.style.display).toBe("none");
  });

  /**
   * Test: Re-sync button is initially hidden
   * Validates: Requirements 3.6
   */
  test("should have Re-sync button initially hidden", () => {
    const resyncBtn = document.getElementById("vpnResyncButton");
    expect(resyncBtn?.style.display).toBe("none");
  });

  /**
   * Test: Status and error displays are initially hidden
   * Validates: Requirements 3.3
   */
  test("should have status and error displays initially hidden", () => {
    const status = document.getElementById("vpnSyncStatus");
    const error = document.getElementById("vpnSyncError");
    expect(status?.style.display).toBe("none");
    expect(error?.style.display).toBe("none");
  });

  /**
   * Test: Tab container has exactly two tabs (Search and Coordinates)
   * Validates: Requirements 3.1 (updated for vpn-toggle-ui)
   */
  test("should have exactly two tabs in the input-mode-tabs container", () => {
    const tabContainer = document.querySelector(".input-mode-tabs");
