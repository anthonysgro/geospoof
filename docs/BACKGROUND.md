...
[Content scripts](https://developer.mozilla.org/en-US/docs/Mozilla/Add-ons/WebExtensions/Content_scripts) cannot directly inject into [`Worker`](https://developer.mozilla.org/en-US/docs/Web/API/Worker), [`SharedWorker`](https://developer.mozilla.org/en-US/docs/Web/API/SharedWorker), or [`ServiceWorker`](https://developer.mozilla.org/en-US/docs/Web/API/ServiceWorker) contexts — they run in separate global scopes with no shared state. GeoSpoof mitigates this on two tiers:

By default on every supported browser, the content script wraps `window.Worker` and `window.SharedWorker` and prepends a self-contained timezone spoofing payload to classic-worker sources. This covers blob-URL, URL-loaded, data-URL, `importScripts`-loaded, and nested workers. Module workers and service workers pass through unpatched under this default configuration because blob-URL wrapping breaks module `import` resolution and service workers require a stable browser-managed URL.

**Advanced Worker Protection** (Firefox only, opt-in) uses [`webRequest.filterResponseData`](https://developer.mozilla.org/en-US/docs/Mozilla/Add-ons/WebExtensions/API/webRequest/filterResponseData) from the background script to prepend the same payload to module-worker and service-worker script responses at the network layer, closing the remaining gap on Firefox. The setting is opt-in because `filterResponseData` is not available on Chromium (deprecated) or Safari (never implemented), and because Subresource Integrity hash checks fail on modified scripts — when SRI is detected via `onerror` the filter disconnects and the original response falls through unmodified so the site continues to function.

When Advanced Worker Protection is enabled, CreepJS's `code:` worker-fingerprint hash matches the main window: the shared payload derives the engine-specific `[native code]` surround format at runtime to match Firefox's multi-line shape, so every spoofed function passes the native-shape check inside workers.
